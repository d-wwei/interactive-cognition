...
with open("cognitive-protocol.md") as f:
    interactive_cognition_rules = f.read()

model = genai.GenerativeModel(
    model_name="gemini-2.0-flash",
    system_instruction=interactive_cognition_rules
)
```
...
- **Optional**: Key sections from `SKILL.md` (cognitive shifts, information flow framework, anti-pattern checklist) if context allows
- **Skip**: Install files, README, full examples

## Stacking with other cognitive bases

Place internal reasoning bases first in the system instruction, then Interactive Cognition. All are concise enough to fit together without significant context cost.
